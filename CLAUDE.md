...
Pull requests targeting `main` trigger `.github/workflows/ci.yml`: three independent jobs (`ai-agent-test`, `backend-test`, `frontend-build`), each `npm ci` + the relevant check (`npm run typecheck && npm test` for the agent, `npm test` for the backend, `npm run build` for the frontend). No Postgres service container and no secrets are needed — the unit tests only cover pure functions (no DB/network access). This is separate from `deploy.yml` below and doesn't run the eval harness (`npm run eval`).

The eval harness is instead wired into its own manually-triggered workflow, `.github/workflows/eval.yml` (`workflow_dispatch` only, not a PR gate — every example makes multiple billed OpenAI calls, too costly/slow/non-deterministic to run on every PR). It spins up an ephemeral `postgres:16` service container (never the real prod `DATABASE_URL` secret), runs Prisma migrations against it, seeds a fixed `eval-test-user` fixture (`backend/scripts/eval-seed.js` — the dataset in `ai-agent-ts/src/eval/dataset.ts` hardcodes this user id for all examples, and without seed data the profile/behavioural/calorie-goal tool calls degrade to a "no data found" fallback instead of the personalized path the reference answers expect), starts the backend, waits on its `/health` endpoint, then runs `npm run eval` in `ai-agent-ts` with `EVAL_FAIL_BELOW=0.7` set so the job actually fails on a real pass-rate regression rather than only on infra errors (`run-eval.ts` exits 0 unconditionally otherwise — this is an opt-in env var, so a developer's local `npm run eval` is unaffected). See `docs/CICD.md`'s "Offline Eval (eval.yml)" section for the full breakdown.
